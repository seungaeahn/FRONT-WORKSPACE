...
function displayImg(page) {
    //현재 페이지 번호('page')와 페이지 당 이미지 수(imagesPerpage)를 곱해서 시작 인덱스를 계산
    //페이지 내에서 표시할 이미지의 첫 번째 이미지 인덱스를 나타냄 
    const startIndex = page * imagesPerpage;
   
    //시작 인덱스에 페이지 당 이미지 수를 더해서 종료 인덱스를 계산 
    //페이지 내에서 표시할 마지막 이미지 다음 인덱스를 나타냄 
    const endIndex = startIndex + imagesPerpage;

    //시작 인덱스에 페이지 당 이미지 수를 더해서 종료 인덱스를 계산
    //페이지 내에서 표시할 이미지의 마지막 이미지 다음 인덱스를 표시
    //이미지 배열('Images')에 시작 인덱스와 종료 인덱스 사이에 이미지를 추출해서 pageImages 배열에 저장
    ...
    pageImages.forEach(images => {
        ...
        //<img> 태그에 src 속성을 현재 이미지의 파일 경로로 설정 
...
function updateButtons() {
    ...
    첫 번째 페이지일 경우에는 이전 버튼을 비활성화 시켜서
    첫 번째 페이지일 경우에는 이전 버튼을 클릭하지 못하게 막는다.
    */
    prevButton.disabled = currentPage ===0;

    /*다음 버튼에서 마지막 페이지일 경우 이미지가 없을 때 
    더이상 다음 버튼이 눌리는 것을 방지하기 위해서 다음 버튼 또한 클릭하지 못하게 막음
    (currentPage + 1) * imagesPerpage : 다음 페이지에 해당하는 이미지의 첫 번째 인덱스
                        imagesPerpage : images 배열의 길이로, 페이지를 넘어갈 수 있는 전체 수를 나타냄 
...
nextButton.addEventListener('click', () => {
    ...
    imagesPerpage : 한 페이지 당 표시되는 이미지의 수
